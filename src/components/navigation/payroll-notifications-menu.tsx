...
export function PayrollNotificationsMenu() {
  const { user } = useAuth()
  ...
  const notificationsQuery = useMyPayrollNotificationsQuery(user?.id, { limit: 8 })
  ...
  const markAllMutation = useMarkAllPayrollNotificationsReadMutation(user?.id, {
  ...
  const handleItemOpen = async (notificationId: string, link?: string | null) => {
    ...
    navigate(ROUTES.PAYROLL_NOTIFICATIONS)
  }
  ...
  return (
    <div className="relative z-50" ref={rootRef}>
      <Button
        variant="ghost"
        size="icon"
        aria-label={t('sidebar.payroll.nav.notifications')}
      ...
      {isOpen ? (
        <div
          className={cn(
            'absolute z-[70] mt-2 w-[min(390px,calc(100vw-1rem))] overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_22px_45px_-25px_rgba(15,23,42,0.55)]',
        ...
        >
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-slate-900">
                  {t('sidebar.payroll.nav.notifications')}
                </p>
            ...
            {notificationsQuery.isPending ? (
              <>
                {Array.from({ length: 5 }).map((_, index) => (
                  <div
                    key={`payroll-notifications-menu-skeleton-${index}`}
              ...
              latestNotifications.length === 0 ? (
                <div className="rounded-xl border border-dashed border-slate-200 bg-muted/20 p-6 text-center">
                  <p className="text-sm font-medium text-slate-900">
                    {t('payroll.dashboard.recentChangesEmptyTitle')}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t('payroll.dashboard.recentChangesEmptyDescription')}
                  </p>
                </div>
              ) : (
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        !notification.isRead
                          ...
                          : 'cursor-pointer hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-200',
                          ...
                          <div className="min-w-0 space-y-1">
                            <div className="flex flex-wrap items-center gap-2">
                              <StatusBadge
                                ...
                                emphasis={!notification.isRead ? 'solid' : 'soft'}
                                className={cn(
                                  'text-[10px]',
                                  !notification.isRead && 'border-white/30 bg-white/10 text-white',
                                )}
                              >
                                {categoryMeta.label}
                              </StatusBadge>
          ...
          <div className="p-3">
            <Button asChild className="w-full" variant="outline" size="sm">
              <Link to={ROUTES.PAYROLL_NOTIFICATIONS} onClick={() => setIsOpen(false)}>
